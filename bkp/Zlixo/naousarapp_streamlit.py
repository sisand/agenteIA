...
menu_grupos = {
    "🔎 Busca & IA": {"🔎 Chat Inteligente": "chat", "🧠 Curadoria IA": "curadoria IA"},
    "📊 Gestão": {"📊 Painel de Gestão": "painel", "🧩 Curadoria Manual": "curadoria"},
...
modo_selecionado_label = st.sidebar.radio(
    "Escolha o modo", rotulos, key="modo_visualizacao"
)
    ...
    try:
        ...
        try:
            ...
            for art in artigos.objects:
                ...
                if isinstance(vetor_ok, list) and all(
                    ...
                    st.warning(
                        f"⚠️ Vetor inválido para o artigo: {art.properties['title']}"
                    )
...
if st.session_state.modo == "curadoria IA":
    ...
    if consulta:
        try:
            ...
            resultados = (
                client.query.get("Article", ["title", "url", "content"])
            ...
            for obj in resultados.objects:
                ...
                artigos_candidatos.append(
                    {"vetor": vetor, "titulo": titulo, "url": url, "conteudo": conteudo}
                )
            ...
            sim_scores = cosine_similarity(
                np.array(embed_consulta).reshape(1, -1), vetores_candidatos
            )[0]

            artigos_ordenados = sorted(
                zip(artigos_candidatos, sim_scores), key=lambda x: x[1], reverse=True
            )[:5]
...
if st.session_state.modo == "chat":
    ...
    with st.form("form_pergunta"):
        pergunta = st.text_input(
            "Digite sua dúvida:", placeholder="Ex: Como emitir uma nota fiscal?"
        )
        imagem = st.file_uploader(
            "Enviar imagem ou print (opcional)", type=["png", "jpg", "jpeg"]
        )
                ...
                if response.status_code == 200:
                    ...
                    if artigos:
                        ...
                        for art in artigos:
                            ...
                            similaridade = art.get("similaridade", None)

                            with st.container():
                                st.markdown(f"#### 🖇️ [{titulo}]({url})")

                                if similaridade is not None:
                                    st.markdown(f"⭐ **Similaridade:** `{similaridade:.2f}%`")
                                    st.progress(int(similaridade))
                    ...
                    st.session_state.historico.insert(
                        0, {"pergunta": pergunta, "resposta": resposta, "tempo": tempo}
                    )
                else:
    ...
    if st.session_state.resposta_atual:
        ...
        feedback_texto = st.text_area(
            "Essa resposta te ajudou? Deseja deixar um comentário?"
        )
        ...
        for idx, h in enumerate(st.session_state.historico):
            with st.expander(f"{idx+1}. {h['pergunta']}"):
                ...
                if idx in st.session_state.feedback:
                    st.markdown(
                        f"**Feedback:** {st.session_state.feedback[idx].capitalize()}"
                    )
 ...
 Resposta do agente:
            ...
            if st.button("💾 Salvar no Weaviate", key=f"salvar_ticket_{ticket['id']}"):
                ...
                if r.status_code == 200:
                    nome_curador = (
                        st.session_state.usuario
                    )  # Substituído pelo usuário logado
                    curadoria_payload = {
                        "ticket_id": ticket["id"],
